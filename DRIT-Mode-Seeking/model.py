...
class DRIT(nn.Module):
  ...
  def forward(self):
    ...
    self.image_display = torch.cat((self.real_A_encoded[0:1].detach().cpu(), self.fake_B_encoded[0:1].detach().cpu(), \
                                    self.fake_B_random[0:1].detach().cpu(), self.fake_B_random2[0:1].detach().cpu(), self.fake_AA_encoded[0:1].detach().cpu(), self.fake_A_recon[0:1].detach().cpu(), \
                                    self.real_B_encoded[0:1].detach().cpu(), self.fake_A_encoded[0:1].detach().cpu(), \
                                    self.fake_A_random[0:1].detach().cpu(), self.fake_A_random2[0:1].detach().cpu(), self.fake_BB_encoded[0:1].detach().cpu(), self.fake_B_recon[0:1].detach().cpu()), dim=0)
  ...
  def update_D(self, image_a, image_b):
    ...
    self.disA2_loss = loss_D2_A.item() + loss_D2_A2.item()
    self.disA2_opt.step()
    ...
    self.disB2_loss = loss_D2_B.item() + loss_D2_B2.item()
    self.disB2_opt.step()
  ...
  def backward_G_alone(self):
    ...
    loss_z_L1 = loss_z_L1_a + loss_z_L1_b + loss_G_GAN2_A + loss_G_GAN2_B + loss_G_GAN2_A2 + loss_G_GAN2_B2 + loss_lz_AB + loss_lz_BA
    loss_z_L1.backward()
  ...
  def assemble_outputs(self):
    ...
    images_a3 = self.normalize_image(self.fake_A_random2).detach()
    ...
    images_b3 = self.normalize_image(self.fake_B_random2).detach()
    images_b4 = self.normalize_image(self.fake_B_recon).detach()
    images_b5 = self.normalize_image(self.fake_BB_encoded).detach()
    row1 = torch.cat((images_a[0:1, ::], images_b1[0:1, ::], images_b2[0:1, ::], images_b3[0:1, ::], images_a5[0:1, ::], images_a4[0:1, ::]),3)
    row2 = torch.cat((images_b[0:1, ::], images_a1[0:1, ::], images_a2[0:1, ::], images_a3[0:1, ::], images_b5[0:1, ::], images_b4[0:1, ::]),3)
